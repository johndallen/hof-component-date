...
module.exports = (key, options) => {
  ...
  const preRender = (req, res, next) => {
    ...
    const conditionalTranslate = (fieldKey) => {
      let result = req.translate(fieldKey);
      if (result === fieldKey) {
        result = null;
      }
      return result;
    };
    const legend = conditionalTranslate(`fields.${key}.legend`);
    const hint = conditionalTranslate(`fields.${key}.hint`);
